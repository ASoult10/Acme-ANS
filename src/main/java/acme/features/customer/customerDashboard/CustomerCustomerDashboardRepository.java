...
public interface CustomerCustomerDashboardRepository extends AbstractRepository {
	...
	@Query("SELECT COALESCE(SUM(f.cost * (SELECT COUNT(bp) FROM BookingPassenger bp WHERE bp.booking.id = b.id)), 0) FROM Booking b JOIN b.flight f WHERE b.customer.id = :customerId AND FUNCTION('YEAR', b.purchaseMoment) = FUNCTION('YEAR', CURRENT_DATE)")
	...
	@Query("SELECT COALESCE(SUM(f.cost * (SELECT COUNT(bp) FROM BookingPassenger bp WHERE bp.booking.id = b.id)), 0) FROM Booking b JOIN b.flight f WHERE b.customer.id = :customerId AND b.purchaseMoment >= CURRENT_DATE - 5 * 365")
	Money bookingTotalCost(Integer customerId);

	@Query("SELECT COALESCE(AVG(f.cost * (SELECT COUNT(bp) FROM BookingPassenger bp WHERE bp.booking.id = b.id)), 0) FROM Booking b JOIN b.flight f WHERE b.customer.id = :customerId AND b.purchaseMoment >= CURRENT_DATE - 5 * 365")
	Money bookingAverageCost(Integer customerId);

	@Query("SELECT COALESCE(MIN(f.cost * (SELECT COUNT(bp) FROM BookingPassenger bp WHERE bp.booking.id = b.id)), 0) FROM Booking b JOIN b.flight f WHERE b.customer.id = :customerId AND b.purchaseMoment >= CURRENT_DATE - 5 * 365")
	Money bookingMinimumCost(Integer customerId);

	@Query("SELECT COALESCE(MAX(f.cost * (SELECT COUNT(bp) FROM BookingPassenger bp WHERE bp.booking.id = b.id)), 0) FROM Booking b JOIN b.flight f WHERE b.customer.id = :customerId AND b.purchaseMoment >= CURRENT_DATE - 5 * 365")
	Money bookingMaximumCost(Integer customerId);

	@Query("SELECT COALESCE(STDDEV(f.cost * (SELECT COUNT(bp) FROM BookingPassenger bp WHERE bp.booking.id = b.id)), 0) FROM Booking b JOIN b.flight f WHERE b.customer.id = :customerId AND b.purchaseMoment >= CURRENT_DATE - 5 * 365")
