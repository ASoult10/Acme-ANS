...
public class CustomerCustomerDashboardShowService extends AbstractGuiService<Customer, CustomerDashboard> {
	...
	public void load() {
		...
		Collection<BookingPassenger> bookingPassengers = this.repository.findAllBookingPassengerOf(customerId);
		...
		if (!bookings.isEmpty() && !bookingPassengers.isEmpty()) {
			...
			Map<Booking, Long> numberOfPassengerByBooking = bookingPassengers.stream().collect(Collectors.groupingBy(BookingPassenger::getBooking, Collectors.counting()));
